...
exports.handler = async (event) => {
  ...
  const hash = createHash('md5').update(params.email_address).digest('hex');
  const myHeaders = new Headers();
  myHeaders.append('Content-Type', 'application/json');
  myHeaders.append('Authorization', process.env.MAILCHIMP_API_KEY || '');

  try {
    const memberDataResponse = await fetch(`${process.env.MAILCHIMP_URL}/${hash}`, {
      method: 'get',
      headers: myHeaders,
    });
    const memberData = await memberDataResponse.json();

    if ((memberData && memberData.status === 'pending') || memberData.status === 'subscribed') {
